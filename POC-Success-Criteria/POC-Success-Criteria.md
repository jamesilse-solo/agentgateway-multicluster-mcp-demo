...
for L7 Agent Gateway.","NOT IN SCOPE — Each cluster runs an independent istiod. A shared/hybrid control plane is not part of this POC deployment.","NOT IN SCOPE: This POC runs independent istiod instances on cluster1 and cluster2 — there is no single shared control plane. Each cluster manages its own ztunnel and AGW data plane independently. Cross-cluster connectivity is handled by east-west gateways, not a unified control plane.

Removed from validate.sh: 2026-04-29.","[Phase7-ControlPlane-Registry-and-Mesh/validate.sh — GitHub](https://github.com/jamesilse-solo/agentgateway-multicluster-mcp-demo/blob/main/POC-Success-Criteria/Phase7-ControlPlane-Registry-and-Mesh/validate.sh)"
...
bound via Kubernetes RBAC.","NOT IN SCOPE — Gloo Mesh Enterprise Workspace CRDs are not deployed in this POC. Admin isolation is out of scope.","NOT IN SCOPE: This POC does not deploy Gloo Mesh Enterprise or Workspace CRDs. Administrative isolation boundaries (Workspace + WorkspaceSettings) require the GME management plane, which is not part of this deployment.
